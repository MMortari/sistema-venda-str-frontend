...
import { VendasInterface } from '../../../interfaces/api'
...
class Vendas extends Component {
  ...
  async componentDidMount() {
    ...
    const vendas: VendasInterface[] = response.data;
    this.setState({ vendas });
    console.log("Vendas -> ", vendas)
  }

  handleDeleteVenda = async (id: number) => {
  ...
  handleInfoVenda(venda: VendasInterface) {
    ...
    return (
      <Fragment>
        ...
        <table className="table table-borded">
          ...
          <tbody>
            { this.state.vendas.map((vendas: VendasInterface, index) => (
              <tr key={index}>
                <td>{vendas.id}</td>
                <td>{vendas.total}</td>
                <td><b>{vendas.produtosId.length}</b></td>
